...
import 'package:multi_crop_picker/crop/crop_render.dart';
import 'package:multi_crop_picker/crop/matrix_decomposition.dart';
import 'package:multi_crop_picker/crop/rotation_gesture_detector.dart';
...
class _CropState extends State<Crop> with TickerProviderStateMixin {
  ...
  void _reCenterImage([bool animate = true]) {
    ...
    final ratio = sz.aspectRatio;//childWidgetSize.aspectRatio / sz.aspectRatio;

    if (sz.aspectRatio < 1.0) {
      // Vertical image. Height needs to be rescaled according to ratio.
      w = sz.width;
      h = min(sz.height * ratio,MediaQuery.of(context).size.height);
      
    } else {
      // Horizontal or square image. Width needs to be rescaled according to ratio (for square ratio is 1.0).      
      w = min(sz.width * ratio,MediaQuery.of(context).size.width);
      h = sz.height;
    }
